 ...
 * @template T
 * @param {T | undefined} initData - Can be data, function, or async function
 * @param {'object' | 'array'} mode - Storage mode
 * @param {T} defaultItem - Default schema item
 */
function resolveInitData(initData, mode, defaultItem) {
	let resolvedData;

	if (initData !== undefined) {
		resolvedData = initData;
	} else {
		resolvedData = mode === 'array' ? /** @type {T[]} */ ([]) : defaultItem;
	}
 ...
 * });
 *
 * // Array storage with function init data
...
export class StoredDataObject {
	 ...
	 * @param {S} schemaDefinition - Schema created with defineSchema() function
	 * @param {Mode} mode - Storage mode
	 * @param {StoredDataOptions<'array' extends Mode ? SchemaToType<S>[] : SchemaToType<S>>} [options] - Configuration options
	 *
	...
	static async from(filePath, schemaDefinition, mode, options = {}) {
		const { encoding = 'utf8', autoValidate = true, initData } = options;
		const absPath = path.resolve(filePath);

		// Create default value
		const defaultItem = createDefaultFromSchema(schemaDefinition);
		const defaultValue = resolveInitData(initData, mode, defaultItem);
		...
		// Validate data
		const validatedData = validateParsedData(parsedData, schemaDefinition, mode, autoValidate);
		...
		return {
			...
			async reload() {
				await lock.run(async () => {
					...
					const newValidatedData = validateParsedData(newParsedData, schemaDefinition, mode, autoValidate);
			 ...
			 *  ? SchemaToType<S>[]
			 *  : SchemaToType<S>
			 * } [newInitData] - Optional new init data to use instead of original
			 */
			async reset(newInitData) {
				await lock.run(async () => {
					const resetValue = newInitData !== undefined ? resolveInitData(newInitData, mode, defaultItem) : defaultValue;

					const resetValidatedData = validateParsedData(resetValue, schemaDefinition, mode, autoValidate);
