...
export class StoredDataObject {
	 ...
	 * @returns {Promise<{data: T extends any[] ? T : Partial<T>, write: () => Promise<void>, reload: () => Promise<void>, filePath: string}>}
	 */
	static async from(filePath, schema, { encoding = 'utf8' } = {}) {
		const absPath = path.resolve(filePath);

		// Create default value based on schema type
		const defaultValue = Array.isArray(schema) ? [] : {};
		...
		try {
			...
			await fs.writeFile(absPath, JSON.stringify(defaultValue, null, 4), encoding);
		}
		...
		let data;
		try {
			data = raw.trim() ? JSON.parse(raw) : defaultValue;
		} catch (err) {
		...
		return {
			...
			async reload() {
				await lock.run(async () => {
					...
					let newData;
					try {
						newData = rawReload.trim() ? JSON.parse(rawReload) : defaultValue;
					} catch (err) {
